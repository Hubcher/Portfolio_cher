...
START_TEST(test_implicate_random_figure) {
  ...
  execute_user_command_and_update_game_state(get_signal(signal), &stats, &curr_position);
...
START_TEST(test_execute_user_command_and_update_game_state) {
  GameInfo_t stats;
  int signal = 0;
  curr_position_figure curr_position;
  ...
  execute_user_command_and_update_game_state(state, &stats, &curr_position);
  signal = KEY_DOWN;
  state = get_signal(signal);
  execute_user_command_and_update_game_state(state, &stats, &curr_position);
  signal = KEY_LEFT;
...
START_TEST(test_clear_intro_message) { clear_intro_message(); }
END_TEST
...
Suite *suite_create_tetris(void) {
  ...
  tcase_add_test(tcase_core, test_execute_user_command_and_update_game_state);
  ...
  tcase_add_test(tcase_core, test_clear_intro_message);
