...
void print_overlay(void) {
  ...
  mvprintw(BOARD_N + 5, BOARD_M - 3, INTRO_MESSAGE);
...
void clear_intro_message(void) {
    int y = BOARD_N + 5; 
    int x = (BOARD_M - 3); 
    for (int i = 0; i < INTRO_MESSAGE_LEN; i++) {
        mvprintw(y, x + i, " ");  
    }
    refresh(); 
}

void print_rectangle(int top_y, int bottom_y, int left_x, int right_x) { // прямоугольник
...
void print_end_game(GameInfo_t *stats) {
  clear();  // Очищаем экран
  printw("Game Over!\nYour score: %d\nPress  ENTER key to exit.", stats->score);
  refresh();
